...
public class MhomeController {
	...
	public ResponseEntity<String> list(@RequestParam(required = true, defaultValue = "0") int sort,@RequestParam(required = true, defaultValue = "0") int type) {
		...
		if (type == 4) {
	        // 개봉작일 경우, 개봉작에 해당하는 개수 계산
	        totalcnt = movieService.getTotalCountForType("개봉");
	        json.put("totalcnt", totalcnt);
	    } else if (type == 5) {
	        // 개봉예정작일 경우, 개봉예정작에 해당하는 개수 계산
	        totalcnt = movieService.getTotalCountForType("개봉예정");
	        json.put("totalcnt", totalcnt);
	    } else {
	    	totalcnt = movieService.getTotalCount();
	    	 json.put("totalcnt", totalcnt);
	    }
